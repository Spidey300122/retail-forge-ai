// backend/ai-engine/compliance/rules/designRules.js - COMPLETE FILE

import { ComplianceRule } from '../ComplianceRule.js';

/**
 * Rule 1: Minimum Font Size
 */
...
/**
 * Rule 2: WCAG Contrast Standards
 */
...
/**
 * Rule 5: Drinkaware Logo for Alcohol
 */
...
/**
 * Rule 6: Background Color Validation
 */
...
/**
 * Rule 10: LEP Design Requirements (COMPLETE)
 */
export class LEPDesignRule extends ComplianceRule {
  ...
  validate(creativeData) {
    // Only apply LEP rules if explicitly marked as LEP
    ...
    // Rule 1: White background enforced
    if (creativeData.backgroundColor !== WHITE && creativeData.backgroundColor !== 'white') {
      violations.push({
        issue: 'backgroundColor',
        current: creativeData.backgroundColor,
        required: WHITE,
        message: 'LEP requires white background only'
      });
    }

    // Rule 2: Tesco Blue font enforced for all text
      ...
      .forEach((el, index) => {
        if (el.fill?.toLowerCase() !== TESCO_BLUE.toLowerCase()) {
          violations.push({
            ...
            current: el.fill,
            required: TESCO_BLUE,
            message: 'LEP requires Tesco Blue (#00539F) for all text'
          });
        }

        // Rule 3: Left-aligned copy enforced
        if (el.originX !== 'left' && el.textAlign !== 'left') {
          violations.push({
            element: `text_${index}`,
            issue: 'textAlignment',
            current: el.originX || el.textAlign,
            required: 'left',
            message: 'LEP requires left-aligned text'
          });
        }
      });

    // Rule 4: White value tile enforced
    if (creativeData.valueTile && creativeData.valueTile.type !== 'white') {
      violations.push({
        issue: 'valueTile',
        current: creativeData.valueTile.type,
        required: 'white',
        message: 'LEP requires white value tile'
      });
    }

    // Rule 5: No branded content other than logo
    const hasBrandedContent = (creativeData.elements || []).some(el => 
      el.isBranded && !el.isLogo
    );

    if (hasBrandedContent) {
      violations.push({
        issue: 'brandedContent',
        message: 'LEP allows no branded content other than logo'
      });
    }

    // Rule 6: LEP logo must be positioned to right
    const lepLogo = (creativeData.elements || []).find(el => el.isLEPLogo);
    if (lepLogo && lepLogo.left < creativeData.canvasWidth / 2) {
      violations.push({
        issue: 'lepLogoPosition',
        message: 'LEP logo must be positioned to right of packshot'
      });
    }

    // Rule 7: Mandatory tag check
    const hasTag = (creativeData.elements || []).some(el => 
      el.type === 'text' && 
      el.text?.toLowerCase().includes('selected stores') &&
      el.text?.toLowerCase().includes('while stocks last')
    );

    if (!hasTag) {
      violations.push({
        issue: 'mandatoryTag',
        message: 'LEP requires tag: "Selected stores. While stocks last"'
      });
    }
    ...
    if (violations.length > 0) {
      return {
        ...
        suggestion:
          'LEP must use: white background, Tesco Blue text (#00539F), white value tile, left-aligned copy, LEP logo right of packshot, no branded content except logo, mandatory tag at bottom',
        affectedElements: violations,
      };
    }
